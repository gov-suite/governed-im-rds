...
export class ValidateJsonSchemaTypeFunc
  ...
  implements rds.StoredFunctionCodeSupplier<ValidateJsonSchemaTypeFunc> {
  constructor(
    readonly validateJsonSchemaTypeParams: gimTyp.TypicalInfoModelStructParams,
  ) {
    super(
      gimc.entityName("_validate_json_schema_type"),
      validateJsonSchemaTypeParams.storedFuncParams,
    );
  }

  storedFunctionCode(
    ctx: rds.RdbmsEngineContext,
    fn: rds.StoredFunction<ValidateJsonSchemaTypeFunc>,
  ): rds.StoredRoutineBodyCode | rds.StoredRoutineCode {
    return {
      isStoredRoutineCode: true,
      persistAsName: "fn-validate-json-schema-type",
      sourceCode: rds.interpolateEntityAttrNamesInSQL(ctx, {
        sql: `
      CREATE OR REPLACE FUNCTION {self}(type text, data jsonb) RETURNS boolean AS
      $f$
        BEGIN
...
export class ValidateJsonSchemaFunc extends rdsTyp.TypicalStoredRoutineEntity
  implements rds.StoredFunctionCodeSupplier<ValidateJsonSchemaFunc> {
  constructor(
    readonly validateJsonSchemaParams: JsonSchemaFactoryParams,
  ) {
    super(
      gimc.entityName("validate_json_schema"),
      validateJsonSchemaParams.storedFuncParams,
    );
  }

  storedFunctionCode(
    ctx: rds.RdbmsEngineContext,
    fn: rds.StoredFunction<ValidateJsonSchemaFunc>,
  ): rds.StoredRoutineBodyCode | rds.StoredRoutineCode {
    return {
      isStoredRoutineCode: true,
      persistAsName: "fn-validate-json-schema",
      sourceCode: rds.interpolateEntityAttrNamesInSQL(ctx, {
        sql: `
        CREATE OR REPLACE FUNCTION {self}(schema jsonb, data jsonb, root_schema jsonb DEFAULT NULL) RETURNS boolean AS
        $f$
        DECLARE
          prop text;
          item jsonb;
        ...
        BEGIN
          ...
          IF schema ? 'type' THEN
            ...
            END IF;
            IF (SELECT NOT bool_or({validateJsonSchemaType}(type, data)) FROM unnest(types) type) THEN
              RETURN false;
            END IF;
          END IF;
          ...
          IF schema ? 'properties' THEN
            FOR prop IN SELECT jsonb_object_keys(schema->'properties') LOOP
              IF data ? prop AND NOT {self}(schema->'properties'->prop, data->prop, root_schema) THEN
                RETURN false;
              END IF;
            END LOOP;
          END IF;
          ...
          IF schema ? 'items' AND jsonb_typeof(data) = 'array' THEN
            IF jsonb_typeof(schema->'items') = 'object' THEN
              FOR item IN SELECT jsonb_array_elements(data) LOOP
                IF NOT {self}(schema->'items', item, root_schema) THEN
                  RETURN false;
                END IF;
              END LOOP;
            ELSE
              IF NOT (
                SELECT bool_and(i > jsonb_array_length(schema->'items') OR {self}(schema->'items'->(i::int - 1), elem, root_schema))
          ...
          IF jsonb_typeof(schema->'additionalItems') = 'object' THEN
            IF NOT (
                SELECT bool_and({self}(schema->'additionalItems', elem, root_schema))
                FROM jsonb_array_elements(data) WITH ORDINALITY AS t(elem, i)
          ...
          IF schema ? 'anyOf' THEN
            IF NOT (SELECT bool_or({self}(sub_schema, data, root_schema)) FROM jsonb_array_elements(schema->'anyOf') sub_schema) THEN
              RETURN false;
            END IF;
          END IF;
        
          IF schema ? 'allOf' THEN
            IF NOT (SELECT bool_and({self}(sub_schema, data, root_schema)) FROM jsonb_array_elements(schema->'allOf') sub_schema) THEN
              RETURN false;
            END IF;
          END IF;
        
          IF schema ? 'oneOf' THEN
            IF 1 != (SELECT COUNT(*) FROM jsonb_array_elements(schema->'oneOf') sub_schema WHERE {self}(sub_schema, data, root_schema)) THEN
              RETURN false;
            END IF;
          END IF;
          ...
          IF schema ? 'additionalProperties' AND jsonb_typeof(data) = 'object' THEN
            ...
            IF jsonb_typeof(schema->'additionalProperties') = 'boolean' THEN
              ...
              END IF;
            ELSEIF NOT (
              SELECT bool_and({self}(schema->'additionalProperties', data->key, root_schema))
          ...
          IF schema ? '$ref' THEN
            ...
            IF NOT {self}(root_schema #> path[2:array_length(path, 1)], data, root_schema) THEN
              RETURN false;
            END IF;
          END IF;
          ...
          IF schema ? 'not' THEN
            IF {self}(schema->'not', data, root_schema) THEN
              RETURN false;
            END IF;
          END IF;
            ...
            FOR prop IN SELECT jsonb_object_keys(schema->'dependencies') LOOP
              IF data ? prop THEN
                IF jsonb_typeof(schema->'dependencies'->prop) = 'array' THEN
                  ...
                  END IF;
                ELSE
                  IF NOT {self}(schema->'dependencies'->prop, data, root_schema) THEN
                    RETURN false;
                  END IF;
                END IF;
              END IF;
          ...
          IF schema ? 'patternProperties' AND jsonb_typeof(data) = 'object' THEN
            FOR prop IN SELECT jsonb_object_keys(data) LOOP
              FOR pattern IN SELECT jsonb_object_keys(schema->'patternProperties') LOOP
                ...
                IF prop ~ pattern AND NOT {self}(schema->'patternProperties'->pattern, data->prop, root_schema) THEN
                  RETURN false;
                END IF;
              END LOOP;
            END LOOP;
      ...
      $f$ LANGUAGE 'plpgsql' IMMUTABLE;
        ...
        with: {
          ...
          validateJsonSchemaType:
            this.validateJsonSchemaParams.jsonSchemaFactory
              .validateJsonSchemaType,
        },
        unindentBlock: true,
      }),
...
export interface JsonSchemaFactoryParams
  extends gimTyp.TypicalInfoModelStructParams {
  readonly jsonSchemaFactory: JsonSchemaFactory;
}

export class JsonSchemaFactory {
  readonly validateJsonSchemaType: ValidateJsonSchemaTypeFunc;
  readonly validateJsonSchema: ValidateJsonSchemaFunc;
  constructor(
    params: gimTyp.TypicalInfoModelStructParams,
  ) {
    const jsonSchemaParams = { ...params, jsonSchemaFactory: this };

    this.validateJsonSchemaType = new ValidateJsonSchemaTypeFunc(params);
    this.validateJsonSchema = new ValidateJsonSchemaFunc(jsonSchemaParams);
  }
}
