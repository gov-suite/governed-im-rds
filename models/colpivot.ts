...
export class CreateColpivotFunc extends rdsTyp.TypicalStoredRoutineEntity
  implements rds.StoredFunctionCodeSupplier<CreateColpivotFunc> {
  constructor(
    readonly colpivotFnParams: gimTyp.TypicalInfoModelStructParams,
  ) {
    super(
      gimc.entityName("colpivot"),
      colpivotFnParams.storedFuncParams,
    );
  }

  storedFunctionCode(
    ctx: rds.RdbmsEngineContext,
    fn: rds.StoredFunction<CreateColpivotFunc>,
  ): rds.StoredRoutineBodyCode | rds.StoredRoutineCode {
    return {
      isStoredRoutineCode: true,
      persistAsName: "fn-create-colpivot",
...
export class ColpivotFactory {
  readonly createColpivotFunc: CreateColpivotFunc;
  constructor(
    params: gimTyp.TypicalInfoModelStructParams,
  ) {
    this.createColpivotFunc = new CreateColpivotFunc(params);
  }
}
